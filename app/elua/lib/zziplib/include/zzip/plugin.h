...
struct zzip_plugin_io { /* use "zzip_plugin_io_handlers" in applications !! */
    int          (*zzip_open)(zzip_char_t* name, int flags, ...);
    int          (*zzip_close)(int fd);
    zzip_ssize_t (*zzip_read)(int fd, void* buf, zzip_size_t len);
    zzip_off_t   (*zzip_seeks)(int fd, zzip_off_t offset, int whence);
    zzip_off_t   (*filesize)(int fd);
    long         sys;
    long         type;
    zzip_ssize_t (*zzip_write)(int fd, _zzip_const void* buf, zzip_size_t len);
};
