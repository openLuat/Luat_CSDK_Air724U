    ...
    while (1)                   /* outer loop */
    {
        ...
        } else
        {
          non_mmap:
            ...
            if (io->fd.zzip_seeks(fd, offset, SEEK_SET) < 0)
                { return(ZZIP_DIR_SEEK); }
            if (io->fd.zzip_read(fd, buf, (zzip_size_t) maplen) < maplen)
...
#     endif
        ...
        {
            if (io->fd.zzip_seeks(fd, zz_rootseek + zz_offset, SEEK_SET) < 0)
                return ZZIP_DIR_SEEK;
            if (io->fd.zzip_read(fd, &dirent, sizeof(dirent)) < __sizeof(dirent))
                return ZZIP_DIR_READ;
            d = &dirent;
        }
...
#       endif
        ...
        if (fd_map)
            ...
            { io->fd.zzip_read(fd, hdr->d_name, u_namlen); }
                ...
                zzip_strings_t * ext, zzip_plugin_io_t io)
{
    ...
    {
        strcpy(file + len, *ext);
        fd = (io->fd.zzip_open)(file, filemode);
        if (fd != -1)
            return fd;
    }
    return -1;
                     ...
                     zzip_strings_t * ext, zzip_plugin_io_t io)
{
    ...
    if (! ext)
        ext = zzip_get_default_ext();
    fd = (io->fd.zzip_open)(filename, O_RDONLY | O_BINARY);
    if (fd != -1)
    {
