...
# endif
#else
#define tells(fd) zzip_seeks(fd,0,SEEK_CUR)
#endif
...
zzip_file_saveoffset(ZZIP_FILE * fp)
{
    ...
    {
        int fd = fp->dir->fd;
        zzip_off_t off = fp->io->fd.zzip_seeks(fd, 0, SEEK_CUR);
...
zzip_file_open(ZZIP_DIR * dir, zzip_char_t * name, int o_mode)
{
    ...
    while (1)
    {
        ...
        if (! filename_strcmp(hdr_name, name))
        {
            ...
            if (dir->io->fd.zzip_seeks(dir->fd, hdr->d_off, SEEK_SET) < 0)
                { err = ZZIP_DIR_SEEK; goto error; }

            {
                ...
                dataoff = dir->io->fd.zzip_read(dir->fd, (void *) p, sizeof(*p));
                ...
                if (dir->io->fd.zzip_seeks(dir->fd, dataoff, SEEK_CUR) < 0)
                    { err = ZZIP_DIR_SEEK; goto error; }
...
zzip_fclose(ZZIP_FILE * fp)
{
    if (! fp)
        return 0;
    if (! fp->dir)               /* stat fd */
        { int r = fp->io->fd.zzip_close(fp->fd); free(fp); return r; }
    else
...
zzip_file_read(ZZIP_FILE * fp, void *buf, zzip_size_t len)
{
    ...
    {
        if (zzip_file_saveoffset(dir->currentfp) < 0
            || fp->io->fd.zzip_seeks(dir->fd, fp->offset, SEEK_SET) < 0)
    ...
    if (fp->method)             /* method != 0   == 8, inflate */
    {
        ...
        do
        {
            ...
            if (fp->crestlen > 0 && fp->d_stream.avail_in == 0)
            {
                 ...
                 */
                zzip_ssize_t i = fp->io->fd.zzip_read(dir->fd, fp->buf32k, cl);
    ...
    {                           /* method == 0 -- unstore */
        rv = fp->io->fd.zzip_read(dir->fd, buf, l);
        if (rv > 0)
            { fp->restlen-= rv; }
        else if (rv < 0)
            { dir->errcode = ZZIP_DIR_READ; }
...
zzip_read(ZZIP_FILE * fp, void *buf, zzip_size_t len)
{
    if (! fp)
        return 0;
    if (! fp->dir)
        { return fp->io->fd.zzip_read(fp->fd, buf, len); }    /* stat fd */
    else
    {
                    ...
                    zzip_strings_t * ext, zzip_plugin_io_t io)
{
  ...
  try_real:
    /* prefer an existing real file */
    {
        ...
        int fd = (os->fd.zzip_open)(filename, o_flags);        /* io->fd.open */
        ...
        if (fd != -1)
        {
            ...
            if (! fp)
                { os->fd.zzip_close(fd); return 0; }  /* io->fd.close */
    ...
    /* see if we can open a file that is a zip file */
    {
        ...
        while ((p = strrchr(basename, '/')))
        {
            ...
            *p = '\0';
            ...
            if (e)
                { errno = zzip_errno(e); io->fd.zzip_close(fd); return 0; }
...
zzip_rewind(ZZIP_FILE * fp)
{
    ...
    if (! fp->dir)
    {                           /* stat fd */
        fp->io->fd.zzip_seeks(fp->fd, 0, SEEK_SET);
        return 0;
    }
    ...
    if (fp->io->fd.zzip_seeks(dir->fd, fp->dataoffset, SEEK_SET) < 0)
        return -1;
...
zzip_seek(ZZIP_FILE * fp, zzip_off_t offset, int whence)
{
    ...
    if (! fp->dir)
    {                           /* stat fd */
        return fp->io->fd.zzip_seeks(fp->fd, offset, whence);
    }
    ...
    {                           /* unstore, just lseek relatively */
        ...
        ofs = fp->io->fd.zzip_seeks(dir->fd, read_size, SEEK_CUR);
        if (ofs > 0)
